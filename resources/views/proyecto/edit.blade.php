    ...
    <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <h2 class="text-2xl font-bold text-center text-gray-800 mb-8">{{ __('Editar Proyecto') }}</h2>
                ...
                <form method="POST" action="{{ route('proyecto.update', $proyecto) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input id="titulo" type="text" 
                            ...
                            name="titulo" value="{{ old('titulo', $proyecto->titulo) }}" required>
                        @error('titulo')
                    ...
                    <div>
                        ...
                        <textarea id="descripcion" 
                            ...
                            name="descripcion" rows="4" required>{{ old('descripcion', $proyecto->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <select id="id_asignatura" name="id_asignatura" 
                            ...
                            @foreach($asignaturas as $asignatura)
                                <option value="{{ $asignatura->id_asignatura }}" 
                                    {{ old('id_asignatura', $proyecto->id_asignatura) == $asignatura->id_asignatura ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input id="grupo" type="text" 
                            ...
                            name="grupo" value="{{ old('grupo', $proyecto->grupo) }}" required>
                        @error('grupo')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="fecha_inicio" type="date" 
                            ...
                            name="fecha_inicio" value="{{ old('fecha_inicio', $proyecto->fecha_inicio->format('Y-m-d')) }}" required>
                        @error('fecha_inicio')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input id="fecha_fin" type="date" 
                            ...
                            name="fecha_fin" value="{{ old('fecha_fin', $proyecto->fecha_fin->format('Y-m-d')) }}" required>
                        @error('fecha_fin')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select id="estado" name="estado" 
                            ...
                            <option value="en_curso" {{ old('estado', $proyecto->estado) == 'en_curso' ? 'selected' : '' }}>En Curso</option>
                    ...
                    <div class="flex items-center justify-end mt-6">
                        <a href="{{ route('proyecto.show', $proyecto) }}" class="mr-4 text-gray-600 hover:text-gray-800">
                            {{ __('Cancelar') }}
                        </a>
